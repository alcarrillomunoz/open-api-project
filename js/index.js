...
.then(data => {
    ...
    /* get artist names from array and create unique array to avoid duplicates and remove null results */
    for (let i = 0; i < 12; i++) {
...
function clearArt () {     
    for (let y = 0; y < artwork.length; y++) {
        let image = displayArt.querySelector("div");
...
function selectArtist() {
    ...
    workByArtistMessage.innerHTML = `Work by <b>${clicked}<b>`;
    ...
    for(let m = 0; m < 10; m++) {
        ...
        .then(data => {
            ...
            if (artData !== null){
             ...
             displayArt.appendChild(artContainer); 
            }
            if (artData === null){
                artwork.length-=1;
            }
